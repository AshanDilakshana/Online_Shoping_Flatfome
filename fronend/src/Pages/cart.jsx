...
import { addToCart, getTotal, loadCart } from "../util/cart";
				...
				{cart.map((item, index) => {
                    ...
                    <div key={index} className="w-full h-[120px] bg-white flex relative items-center">
                        ...
                        <div className="w-[200px] h-full  flex flex-col pl-[5px] pt-[10px] ">
                            <h1 className=" font-semibold text-lg w-full text-wrap">{item.productName}</h1>
                            {/* productID */}
                        ...
                        <div className="w-[180px]  h-full flex flex-col">
                            {
                                ...
                                item.LabledPrice>item.productPrice&&
                                <span className="text-secondary w-full  text-right line-through text-lg pr-[10px] mt-[20px]">LKR {item.LabledPrice.toFixed(2)}</span>
                            }
                            <span className="font-semibold text-accent w-full text-right text-2xl pr-[10px] mt-[5px]">LKR {item.productPrice.toFixed(2)}</span>
                        </div>
                    </div>
                    );
				})}
                 <div  className="w-full h-[120px] bg-white flex justify-end items-center relative">
                    <Link state={cart} to="/checkout"
                    ...
                    <div className="h-[50px]">
                        ...
                        Total: LKR {getTotal().toFixed(2)}</span>
                    </div>
                 </div>
			</div>
		</div>
